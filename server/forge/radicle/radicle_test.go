		...
		g.Describe("When requesting projects", func() {
			...
			g.Describe("with many projects", func() {
				g.It("Should succeed", func() {
					...
					g.Assert(repos[2].Name).Equal("c-project")
					g.Assert(repos[2].FullName).Equal("user_login/c-project")
					...
					g.Assert(repos[2].Owner).Equal("user_login")

					g.Assert(repos[3].ForgeRemoteID).Equal(model.ForgeRemoteID("d_project"))
					g.Assert(repos[3].Name).Equal("d-project")
					g.Assert(repos[3].FullName).Equal("user_login/d-project")
					...
					g.Assert(repos[3].Owner).Equal("user_login")
				})
			})
		})
